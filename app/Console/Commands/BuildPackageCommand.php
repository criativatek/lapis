...
class BuildPackageCommand extends Command
{
    ...
    protected function packageList(BuildsSsrBundle $ssrBuilder): ?array
    {
        ...
        // Rebuilds BEFORE either inventory is read. `npm run build:ssr` runs
        // `vite build` first — it replaces public/build wholesale, with new
        // content hashes — before building bootstrap/ssr. Reading
        // compiledAssets() ahead of this rebuild would list a client build
        // this rebuild is about to erase out from under it.
        if (! $ssrBuilder->build()) {
    ...
    /**
     * Inventories `bootstrap/ssr` — called only after `packageList()` has
     * already rebuilt it via `BuildsSsrBundle`, so what this reads is never
     * older than the tree being packaged. See that call site's comment for
     * why the rebuild cannot happen inside this method: it would run after
     * `compiledAssets()` had already read public/build, and the rebuild
     * replaces public/build too.
     *
     * An empty list is not an error: a deploy that renders in the browser is
     * a slower crawl, never a broken page. It IS worth saying out loud,
     * because the alternative is a silent downgrade.
     *
     * @return list<string>
     */
