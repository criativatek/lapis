...
/**
 * O QUE ESTE FORMULÁRIO GRAVA: a etiqueta, o estado, as datas do ano e os seus
 * períodos de avaliação — e nada mais.
 *
 * AS EXCEÇÕES LETIVAS SAÍRAM DAQUI. Durante a Fase 5.4 estiveram neste mesmo
 * `useForm`, e o «Guardar ano letivo» lá em baixo gravava-as com os períodos.
 * Não voltam: cada feriado e cada interrupção tem agora os seus próprios botões
 * de «Editar», «Guardar» e «Eliminar», em ExceptionsManager.vue, e cada um faz
 * um pedido a sério. O que este botão diz que faz e o que ele faz voltaram a ser
 * a mesma coisa.
 *
 * OS PERÍODOS FICARAM, e é deliberado: são dois ou três, nascem com o ano, e a
 * ordem de cada um é uma propriedade do conjunto — mudar um de sequência mexe
 * nos outros. Isso é uma gravação em bloco, e continua a ser uma.
 */
type YearData = {
    label: string;
    starts_on: string;
        ...
        <section class="space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-muted-foreground">
                        Semestres, trimestres ou outra divisão. Cada período tem
                        de estar dentro do ano.
                    </p>
                </div>
                <Button
                    type="button"
            ...
            >
                <div class="grid gap-1.5">
                    <Label :for="`period-label-${index}`" class="text-xs"
                        >Designação</Label
                    >
                    <Input
                        :id="`period-label-${index}`"
                        v-model="period.label"
                        placeholder="Ex.: 1.º Semestre"
                    />
                </div>
                <div class="grid gap-1.5">
                    <Label :for="`period-kind-${index}`" class="text-xs"
                        >Tipo</Label
                    >
                    <select
                ...
                <div class="grid gap-1.5">
                    <Label :for="`period-seq-${index}`" class="text-xs"
                        >Ordem</Label
                    >
                    <Input
                        :id="`period-seq-${index}`"
                        v-model.number="period.sequence"
                        type="number"
                        min="1"
                    />
                </div>
                <div class="grid gap-1.5">
                    <Label :for="`period-start-${index}`" class="text-xs"
                        >Início</Label
                    >
                    <Input
                        :id="`period-start-${index}`"
                        v-model="period.starts_on"
                        type="date"
                    />
                </div>
                <div class="grid gap-1.5">
                    <Label :for="`period-end-${index}`" class="text-xs"
                        >Fim</Label
                    >
                    <Input
                        :id="`period-end-${index}`"
                        v-model="period.ends_on"
                        type="date"
                    />
                </div>
                <Button
                ...
                <InputError
                    class="sm:col-span-6"
                    :message="periodError(index, 'starts_on')"
                />
            </div>
        </section>
