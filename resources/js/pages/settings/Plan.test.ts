...
describe('settings/Plan — trial_expired', () => {
    it('renders no activation button and the already-used message', () => {
        const wrapper = mountPage({ state: 'trial_expired', currentPlanName: 'LÁPIS Base' });
...
describe('settings/Plan — pro_active', () => {
    it('shows the used-before note when usedTrialBefore is true', () => {
        const wrapper = mountPage({ state: 'pro_active', currentPlanName: 'LÁPIS Pro', usedTrialBefore: true });

        expect(wrapper.find('button').exists()).toBe(false);
        expect(wrapper.text()).toContain('LÁPIS Pro');
        expect(wrapper.text()).toContain('anteriormente');
    });

    it('shows no such note when usedTrialBefore is false', () => {
        const wrapper = mountPage({ state: 'pro_active', currentPlanName: 'LÁPIS Pro', usedTrialBefore: false });
