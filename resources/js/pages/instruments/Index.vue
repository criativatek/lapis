...
<template>
    ...
    <div class="mx-auto w-full max-w-4xl space-y-6 p-4">
        <Heading title="Instrumentos" description="Testes, fichas, questões-aula e outras atividades de avaliação." />
        ...
        <div v-if="instruments.length === 0" class="rounded-lg border border-dashed border-border p-10 text-center">
            ...
            <p class="text-sm text-muted-foreground">
                Ainda não tem instrumentos. Crie o primeiro a partir de uma turma.
            </p>
        </div>
        ...
        <div v-else class="overflow-hidden rounded-lg border border-border">
            <table class="w-full text-sm">
                <thead class="bg-muted/50 text-left text-muted-foreground">
                    <tr>
                        <th class="px-4 py-2.5 font-medium">Instrumento</th>
