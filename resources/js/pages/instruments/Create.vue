...
type ItemRow = {
    code: string;
    // '' rather than null: the Input's model type is string|number, and the
    // backend treats an empty string as no label.
    label: string;
...
function submit(): void {
    form.post(`/classes/${props.schoolClass.ulid}/instruments`, { preserveScroll: true });
}
</script>
    ...
    <div class="mx-auto w-full max-w-3xl space-y-6 p-4">
        ...
        <form class="space-y-8" @submit.prevent="submit">
            ...
            <section class="space-y-3">
                ...
                <div v-for="(item, index) in form.items" :key="index" class="space-y-3 rounded-lg border border-border p-3">
            ...
            <Button type="submit" :disabled="form.processing">Criar instrumento</Button>
        </form>
    </div>
</template>
