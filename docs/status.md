...
- **Aulas, sumários, grupos e assiduidade não estão no backup canónico** (`schema_version` 8). Guardar só a assiduidade não teria aula a que se ligar; incluir o módulo de aulas é uma fatia própria (coleções novas + `schema_version` 9 + round-trip). Registado na 0.145.0.
