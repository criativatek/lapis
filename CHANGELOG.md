...
- **A regra passou a ser temporal e por ocorrência.** Um bloco aparece numa semana se, no dia da sua ocorrência nessa semana, `starts_on ≤ dia ≤ ends_on` (NULL = aberto, a mesma regra da materialização) **e** a turma ainda não estava arquivada nesse dia (`archived_at`, no fuso da organização). Uma versão substituída e a sua substituta nunca aparecem juntas; uma versão futura só aparece a partir da semana em que entra em vigor; turmas de apoio válidas aparecem como qualquer outra. A decisão é pelos dados, nunca pelo rótulo nem pela cor. Fronteiras: `starts_on` e `ends_on` são inclusivos; como arquivar grava a hora do momento, uma turma arquivada num dia já não aparece na ocorrência desse mesmo dia (aparece até à véspera).
- **«Configurar manualmente» oferecia turmas arquivadas.** A lista do Horário do Professor é agora configuração operacional atual: só turmas não arquivadas (`archived_at` nulo), incluindo turmas de apoio ativas. O horário semanal continua a ser uma projeção temporal e pode mostrar uma turma arquivada numa semana em que ainda estava ativa, mas essa turma não volta a ser configurável. O teste que exigia que esta lista fosse igual à do seletor `classes.schedule-setup` foi substituído por testes das duas semânticas.
...
- **Cobertura do registo de atividade** — novos eventos no `audit_events` existente: turmas (`class.created/updated/activated/archived/restored/deleted`), grupos T1/T2 (`class_group.*`), inscrições (`enrollment.created/updated/removed/process_numbers_updated/support_reused`, `roster_import.confirmed`), sumário guardado de novo (`lesson.summary_saved`), elementos de avaliação (`instrument.created/updated/deleted/cancelled/cancellation_reverted/correction_completed/correction_reopened`), notas (`scores.recorded`, um por gravação com alterações, também na importação de grelhas), avaliações intercalares (`interim_assessment.*`), registos (`record.created/updated/deleted`, lote de trabalho de casa), autoavaliações (`self_assessment.recorded`). Registados depois da escrita bem-sucedida (dentro da transação onde existe), um por pedido, só com ids e contagens — nunca nomes de alunos, notas ou texto de observações. Sem eventos para páginas, pesquisas, filtros ou rascunhos.
...
> número já tinha sido publicado a partir da outra linha de trabalho — e já
> estava em produção — quando este commit chegou ao `origin`. Duas pessoas
> escolheram o mesmo número porque ambas o escolheram **antes** do último
> `git fetch`; é o único momento em que essa escolha é fiável.
...
> **Nota de reconciliação (0.104.2).** Esta funcionalidade foi implementada
> como 0.102.0 numa branch isolada, entretanto ultrapassada por outro release
> que já tinha reclamado esse número (composição documental dos relatórios).
> Reconciliada sobre a 0.104.1 e renumerada para 0.104.2 — o corpo do trabalho
> não mudou, só o número.
